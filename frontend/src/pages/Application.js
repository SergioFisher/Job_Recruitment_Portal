...
function Application() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/jobRecruitmentPortal/applications')
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:8080/jobRecruitmentPortal/applications', {
      ...
      <Grid container spacing={2}>
        {applications.map(app => (
          <Grid item xs={12} sm={6} key={app.id}>
            <Card>
              <CardContent>
                <Typography variant="subtitle1">JobSeeker ID: {app.jobSeeker_id}</Typography>
                <Typography variant="subtitle2">JobListing ID: {app.jobListing_id}</Typography>
                <Typography variant="body2">Date: {app.applicationDate}</Typography>
